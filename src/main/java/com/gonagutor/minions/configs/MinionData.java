...
public class MinionData implements ConfigurationSerializable {
	...
	public static MinionData deserialize(Map<String, Object> map) {
		try {
			...
			return new MinionData(ChatColor.translateAlternateColorCodes('&', (String) map.get("item_name")), lore,
					...
					Color.deserialize((Map<String, Object>) map.get("minion_color")), (String) map.get("skull_owner"),
	...
	public Map<String, Object> serialize() {
		return ImmutableMap.<String, Object>builder().put("item_name", itemName).put("item_lore", itemLore)
				...
				.put("skull_owner", skullOwner).put("chestplate", chest.getType().toString())
				.put("leggins", legs.getType().toString()).put("boots", boots.getType().toString())
				.put("tool", tool.getType().toString()).build();
	}
	...
	public Boolean isDataValid() {
		if (this.blockMaterial != null && this.dropMaterial != null && this.itemName != null
	...
	public ItemStack toSkull() {
		...
		if (isUrl)
			item = CustomSkull.skullFromImage(item, skullOwner);
