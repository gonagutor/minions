...
public class MinionsCommand implements TabExecutor {
	...
	public static void givePlayerMinion(MinionData minion, Player giveTo, int level) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (args.length == 1) return sendHelpMessage(sender);
		...
		switch (args[0].toLowerCase()) {
			case "give":
				if (sender instanceof Player) {
					try {
						...
						if (args.length < 4) {
							givePlayerMinion(mData, (Player) sender, Integer.parseInt(args[2]));
							return true;
						}
						Player sendTo = Bukkit.getPlayerExact(args[3]);
						if (sendTo != null) {
							givePlayerMinion(mData, sendTo, Integer.parseInt(args[2]));
							((Player) sender).sendMessage(Minions.getPrefix() + args[3] + " received " + mData.getItemName());
							return true;
						}
						((Player) sender).sendMessage(Minions.getPrefix() + ChatColor.RED + "Could not find player named " + args[3]);
					} catch (Exception e) {
						e.printStackTrace();
			...
			case "list":
				...
				if (args.length < 2) {
					sender.sendMessage(ChatColor.GREEN +"==========" + ChatColor.GRAY + " Available Minions (1/" + maxPages + ")" + ChatColor.GREEN + " ==========");
				...
				try {
					...
					sender.sendMessage(ChatColor.GREEN + "==========" + ChatColor.GRAY + " Available Minion (" + pageNumber + "/" + maxPages + ")" + ChatColor.GREEN + " ==========");
					for (int i = (pageNumber - 1) * 10; i < minionList.size() && (int) Math.ceil(((float) i) / 10) < 10; i++) {
						sender.sendMessage(ChatColor.GOLD + "" + i + " - " + ChatColor.RESET + ((MinionData) minionList.toArray()[i]).getItemName());
					}
					return true;
				} catch (Exception e) {
					sender.sendMessage(Minions.getPrefix() + ChatColor.RED + "That page does not exist. Available pages: " + maxPages);
					return true;
				}
