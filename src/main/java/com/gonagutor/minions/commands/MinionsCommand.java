...
public class MinionsCommand implements TabExecutor {
	...
	private boolean sendHelpMessage(CommandSender sender) {
		...
		sender.sendMessage(
			ChatColor.GOLD + "/minions help " + ChatColor.GRAY + " - Shows this"
		);
		return (true);
	}
	...
	public List<String> onTabComplete(
		...
		if (args.length == 2) {
			for (
				int i = 0;
				i < minionManager.getMinionList().size();
				i++
			) ret.add("" + i);
			return ret;
		}
