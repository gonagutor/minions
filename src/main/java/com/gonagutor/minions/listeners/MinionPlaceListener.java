...
public class MinionPlaceListener implements Listener {
	...
	public void onSkullPlace(BlockPlaceEvent e) {
		...
		if (!e.getItemInHand().getItemMeta().getPersistentDataContainer().has(minionType, PersistentDataType.STRING)) return;
		NamespacedKey minionLevel = new NamespacedKey(Minions.getPlugin(Minions.class), "minion_level");
		if (!e.getItemInHand().getItemMeta().getPersistentDataContainer().has(minionLevel, PersistentDataType.INTEGER)) return;
		MinionData mData = null;

		int level = e.getItemInHand().getItemMeta().getPersistentDataContainer().get(minionLevel, PersistentDataType.INTEGER);
		for (MinionData data : minionManager.getMinionList()) {
			if (data.toSkull(level).getItemMeta().getDisplayName().equals(e.getItemInHand().getItemMeta().getDisplayName())) {
				mData = data;
				break;
			}
		}
		...
		if (e.getBlock().getLocation().subtract(0, 1, 0).getBlock().getType() == Material.AIR) {
			e.getPlayer().sendMessage(Minions.getPrefix() + ChatColor.RED + "You can't place a minion on the air");
			return;
		}

		Location minionLocation = e.getBlock().getLocation().add(0.5, 0, 0.5);
		BaseMinion minion = (BaseMinion) mData.toMinion(minionLocation, level, e.getPlayer().getUniqueId());
